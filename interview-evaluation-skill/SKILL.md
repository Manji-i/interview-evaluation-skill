...
版本记录见 `references/version-history.md`。
...
3. 每次评分前都必须重新确认本次考察维度：
   - 维度统一归入 `professional_ability`（专业能力）和 `soft_quality`（软素质）。
   - 先读取 `templates` 中已保存的维度模版，结合岗位/职级/面试轮次，询问用户本次是沿用某个历史模版、在历史模版上增删维度，还是新建模版。
   - 如果用户已经给出本次维度，先标准化命名，只对歧义项追问，并询问是否保存为新模版或更新已有模版。
   - 如果用户没有给维度，根据岗位和上下文提出一组精简默认维度，请用户确认或调整；不要直接开始评分。
   - 用户确认后的维度写入维度库；如果用户要求复用，使用 `scripts/rubric_memory.py upsert-template` 保存为模版。
...
- `NH`、`H-`、`H`、`H+`、`MH` 的等级锚点。
- 来自用户反馈的评分校准记录。
- 维度模版：一组可复用的本次面试考察维度组合，例如“产品经理一面”“销售岗位业务面”“HRBP 终面”。
...
python3 scripts/rubric_memory.py list-templates
...
python3 scripts/rubric_memory.py upsert-template --name "产品经理一面" --role "产品经理" --description "产品经理一面常用维度" --dimensions-json '[{"category":"professional_ability","name":"业务理解"},{"category":"professional_ability","name":"产品拆解"},{"category":"soft_quality","name":"学习能力"}]'
